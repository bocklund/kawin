...
def plotContributionOverTime(model: PrecipitateModel, strengthModel: StrengthModel, contribution: StrengthContributionBase, phase=None, timeUnits='s', strengthUnits='MPa', ax=None, *args, **kwargs):
    '''
    ...
    if phase is None:
        ...
        phase_idx = strengthModel.phases.index(phase)
    timeScale, timeLabel, bounds = _get_time_axis(model.data.time, timeUnits=timeUnits)
    ax = _plotContributionOverX(timeScale*model.data.time, strengthModel.rss[:,phase_idx], strengthModel.Ls[:,phase_idx], contribution, strengthModel.dislocations, strengthUnits=strengthUnits, ax=ax, *args, **kwargs)
...
def plotPrecipitateStrengthOverTime(model: PrecipitateModel, strengthModel: StrengthModel, phase=None, plotContributions=False, timeUnits='s', strengthUnits='MPa', ax=None, *args, **kwargs):
    ...
    if phase is None:
        ...
        phase_idx = strengthModel.phases.index(phase)
    timeScale, timeLabel, bounds = _get_time_axis(model.data.time, timeUnits=timeUnits)
    ax = _plotPrecipitateStrengthOverX(timeScale*model.data.time, strengthModel.rss[:,phase_idx], strengthModel.Ls[:,phase_idx], strengthModel, phase, plotContributions=plotContributions, strengthUnits=strengthUnits, ax=ax, *args, **kwargs)
    ax.set_xlabel(timeLabel)
    ax.set_xlim(bounds)
    ax.set_xscale('log')
    return ax
